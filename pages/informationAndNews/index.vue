<template>
  <main class="min-h-screen">
    ...
    <!-- tabs -->
    <section class="flex flex-col items-center md_1024:(flex flex-row justify-center) my-40px">
      <div v-for="tab in newsAndInfoTabs" :key="tab.id" @click="checkedTabId = tab.id"
        class="w-fit cursor-pointer flex flex-col text-50px transition duration-300 py-16ppx px-8px relative hover:sub-color"
      ...
      >
        ...
        <div 
          class="w-10px h-10px rounded-full bg-sub absolute left-0px top-0px transition duration-300"
          :class="{'block':checkedTabId===tab.id,'hidden':checkedTabId!==tab.id}"
        ></div>
        
        <!-- 編匡 -->
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 350 150"
          class="absolute w-350px h-180px -top-15px left-0 transition duration-200" 
          :class="{'block':checkedTabId===tab.id,'hidden':checkedTabId!==tab.id}">
